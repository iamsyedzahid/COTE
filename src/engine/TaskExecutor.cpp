...
void TaskExecutor::execute(std::shared_ptr<Task> task, MetricsService& metrics) {
    ...
    metrics.recordEvent(task->id, task->priority, "RUNNING", task->duration_ms);
    ...
    if (fut.wait_for(task->timeout) == std::future_status::timeout) {
        ...
        metrics.recordEvent(task->id, task->priority, "TIMEOUT", task->duration_ms);
    } else {
        ...
        metrics.recordEvent(task->id, task->priority, "DONE", task->duration_ms);
    }
